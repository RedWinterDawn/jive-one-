               ...
               failure:(void (^)(id responseObject, NSError *error))failure
{
    if (retryCount <= 0) {
        ...
        void (^failureBlock)(AFHTTPRequestOperation *operation, NSError *error) = ^(AFHTTPRequestOperation *operation, NSError *error) {
            ...
            {
                [JCAuthManager requestAuthentication:^(BOOL authenticated, NSError *error) {
                    if (authenticated) {
                        [self requestWithType:type
                                         path:path
                                   parameters:parameters
...
- (NSURLRequest *)requestBySerializingRequest:(NSURLRequest *)request withParameters:(id)object error:(NSError *__autoreleasing *)error
{
    ...
    NSString *authToken = [UIApplication sharedApplication].authenticationManager.authToken.accessToken;
    if (authToken) {
