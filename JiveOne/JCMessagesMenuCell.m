...
-(void)awakeFromNib
{
    ...
    [badgeManager addObserver:self forKeyPath:NSStringFromSelector(@selector(smsMessages)) options:NSKeyValueObservingOptionNew context:NULL];
...
-(void)dealloc {
    JCBadgeManager *badgeManager = [JCBadgeManager sharedManager];
    [badgeManager removeObserver:self forKeyPath:NSStringFromSelector(@selector(smsMessages))];
...
{
    if ([keyPath isEqualToString:NSStringFromSelector(@selector(smsMessages))] /*|| [keyPath isEqualToString:@"missedCalls"]*/) {
